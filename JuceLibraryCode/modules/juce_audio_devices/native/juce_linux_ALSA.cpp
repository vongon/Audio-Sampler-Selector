...
// cpage        unsigned int periods = 2;
        ...
        if (JUCE_ALSA_FAILED (snd_pcm_hw_params_set_channels (handle, hwParams, numChannels))) {
        	return false;
    	}
        if (err = snd_pcm_hw_params_set_periods_near (handle, hwParams, &periods, &dir)) {
        	return false;
        }
        if (JUCE_ALSA_FAILED (snd_pcm_hw_params_set_period_size_near (handle, hwParams, &samplesPerPeriod, &dir))) {
        	return false;
        }
        if (JUCE_ALSA_FAILED (snd_pcm_hw_params (handle, hwParams))) {
            return false;
        }
...
public:
    ...
    int getDefaultBufferSize() override                      { return 16384; }
